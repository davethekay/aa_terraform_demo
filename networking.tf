...
resource "aws_internet_gateway" "igw_aa" {
  ...
  tags = merge(local.common_tags, {
    internetGatewayName = "internet_gateway_public_aa"
    Name                = "INTERNET_GATEWAY_PUBLIC_AA"
  })
}

# Create a route table for the public subnet.
resource "aws_route_table" "route_main_public_aa" {
  vpc_id = aws_vpc.vpc_main_aa.id
  route {
...
# resource "aws_route" "public_to_private_route_aa" {
#   route_table_id            = aws_route_table.route_main_public_aa.id
#   local_gateway_id = nat_gateway_id
